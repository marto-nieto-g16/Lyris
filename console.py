...
class Console(Cmd):
    ...
    def do_scan(self, args):
        ...
        if len(args) == 0:
            print('\033[91m Ingrese una Direccion IP para Empezar el Escanner de la Red')
        else:
            lyris.Scan_Net(args)
            lyris.run_MAC_All()            
    ...
    def do_load_file(self, args):
        ...
        if len(args) != 0:
            lyris.Load_File_MAC_Pertain(args)

    def do_save_file_intruder(self, args):
        """Permite Guardar en un archivo txt las MAC intrusas mas su dirección IP."""
        if len(args) != 0:
            lyris.set_Archivo_MAC_Intruso(args)
        else:
